...
이로써 게시물 목록, 상세 페이지, 수정, 등록, 삭제, 로그인, 댓글, 대댓글 등 구현을 완성했다. <br>


## 02/16
게시물 상세 페이지 컴포넌트에 내용, 댓글, 대댓글 등 모든 기능을 넣어 800줄이 넘어 유지보수에 힘든점이 있었음. <br>
그래서 우선 댓글과 대댓글을 분리하기 위해 src/component 에 폴더 생성 후 이동을 했으며, 대댓글은 이후 추가 작업할 예정임. <br>

약 400줄 이상 줄일 수 있었으며 아직도 해당 댓글 컴포넌트가 길어 좀 더 세분화 해서 분리 작업을 통해 200줄 안으로 줄이는 것을 목표로 하고 있음. <br>
