...
## 02/16
게시물 상세 페이지 컴포넌트의 내용, 댓글, 대댓글 등 모든 기능을 넣어 800줄이 넘어 유지보수에 힘든점이 있었음. <br>
그래서 우선 댓글과 대댓글을 분리하기 위해 src/component 에 폴더 생성 후 이동을 했으며, 대댓글은 이후 추가 작업할 예정임. <br>
생각보다 댓글의 id를 통해 대댓글을 입력하고 위치하는 과정이 길어져 수정하면서도 복잡함을 느낌. <br>

우선 메인 게시물 상세페이지에서 약 400줄 이상 줄일 수 있었으며 아직도 해당 댓글 컴포넌트가 길어 좀 더 세분화 해서 분리 작업을 통해 200줄 안으로 줄이는 것을 목표로 하고 있음. <br>
