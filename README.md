...
1. 게시글, 댓글 추천 기능 - 해당 아이디만 한 번 추천을 하고 싶었으나 sql문을 잘 다루지 못해 한 사람이 여러번 추천 되게 하지 못함
2. 코드가 난잡함 - 컴포넌트를 분리하지를 못 함 - 이후 개선할 것임
3. 해당 fetch 함수 또는 삭제, 수정 함수를 모듈화 하지 못함 - 수정, 삭제, 등록을 커스텀 훅으로 만들어 간략할 것임
